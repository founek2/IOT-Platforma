...
import EditControl from './editControlForm/EditControl'
import Typography from '@material-ui/core/Typography';
import { transformControlForForm } from 'common/lib/utils/transform'
...
class EditDeviceDialog extends Component {
    ...
    preFillForm = device => {
        if (device.control && device.control.recipe) {
            const { fillEditFormAction } = this.props;
            fillEditFormAction(transformControlForForm(device.control.recipe))
        }
    }
    ...
    removeSensorByIndex = id => {
        ...
        dispatch(formsActions.setFormData("EDIT_CONTROL", newEditForm))
    }
    ...
    render() {
        ...
        const handleSave = async () => {
            ...
            await this.props.updateControlAction(device.id)
...
const _mapStateToProps = state => {
    const editForm = getFormData("EDIT_CONTROL")(state);
...
const _mapDispatchToProps = dispatch => (
    {
        ...bindActionCreators(
            {
                updateSensorCount: formsActions.updateFormField("EDIT_CONTROL.count"),
                fillEditFormAction: formsActions.fillForm('EDIT_CONTROL'),
                updateControlAction: deviceActions.updateControl,
            },
            dispatch
        ),
        dispatch,
