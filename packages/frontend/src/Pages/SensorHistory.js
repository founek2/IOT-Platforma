...
function transformData(sensorsData, sensorRecipe, sensorName) {
	...
	const keys = [];
	if (sensorName) {
		// Only data from specific sensor
	...
	sensorsData.forEach(({ samples, nsamples, timestamps, day, sum, min, max }) => {
		...
		if (!sumObject[day])
			sumObject[day] = {
				sum: { day: { ...zeroObj }, night: { ...zeroObj } },
				nsamples: { day: 0, night: 0 },
				min: { ...nullObj },
				max: { ...nullObj },
			};
		...
		for (let j = 0; j < keys.length; j++) {
			...
			if (sum.day) oneDay.sum.day[key] += sum.day[key];

			if (sum.night) oneDay.sum.night[key] += sum.night[key];
...
function Sensors({
	...
	const [dataArray, sumObject] = useMemo(
		() => (hasData ? transformData(sensors.data, device.sensors.recipe, sensorName) : []),
		[hasData && sensors.data[0].first, hasData && sensors.data[sensors.data.length - 1].last]
	); // check from -> to
					...
					<Card className={classes.root}>
						...
						{hasData ? ( // Chart needs two points to draw a line
							<Chart
								data={dataArray}
								vAxisTitle={
									sensorName ? device.sensors.recipe.find(({ name }) => name === sensorName).unit : ""
								}
								hAxisTitle="Čas"
								chartType="LineChart"
							/>
						...
						{hasData && sensorName ? (
							<DetailTable
								sumObject={sumObject}
								sensorRecipe={device.sensors.recipe.find(({ name }) => name === sensorName)}
							/>
						) : null}
						{hasData && !sensorName
							? device.sensors.recipe.map((obj) => (
									<DetailTable sumObject={sumObject} sensorRecipe={obj} key={obj.JSONkey} />
							  ))
							: null}
					</Card>
