...
userSchema.statics.updateUser = async function (userID, data) {
	...
	return this.model("User").findOneAndUpdate({ _id: mongoose.Types.ObjectId(userID) }, { $set: data });
};
...
userSchema.statics.addNotifyToken = function (userID, token) {
	return this.model("User").updateOne({ _id: ObjectId(userID) }, { $addToSet: { notifyTokens: token } });
};
