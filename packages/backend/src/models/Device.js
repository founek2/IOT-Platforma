...
deviceSchema.statics.updateAck = async function (ownerId, topic) {
	const doc = await this.model("Device")
		.findOneAndUpdate(
...
deviceSchema.statics.updateByFormData = function (deviceID, formData, imgExtension, { id, admin }) {
	return this.model("Device")
		...
		.then(async (doc) => {
			...
			if (topic) {
				const result = await this.model("Device").exists({
					topic,
					createdBy: doc.createdBy,
					_id: { $ne: ObjectId(deviceID) },
				});
				if (result) throw Error("topicAlreadyUsed");
			}
			...
			return this.model("Device")
				.updateOne({ _id: ObjectId(deviceID) }, formDataNested)
				.then(() => origImgPath);
		})
		.catch(catcher("device"));
};
...
deviceSchema.statics.getOwnerAndTopic = async function (apiKey) {
	return this.model("Device")
		.findOne({ apiKey }, { createdBy: 1, topic: 1 })
		.lean()
		.then((doc) => {
...
deviceSchema.statics.updateSensorsData = async function (ownerId, topic, data, updateTime) {
	return this.model("Device")
		.findOneAndUpdate(
			...
			{
				fields: {
					"sensors.sampleInterval": 1,
					"sensors.recipe": 1,
					publicRead: 1,
					"sensors.historical.updatedAt": 1,
					"permissions.read": 1,
					"info.title": 1,
				},
			}
		...
		.then((doc) => {
			if (doc) {
				...
				if (
					sensors.sampleInterval !== -1 &&
					(!updatedAt || (new Date() - updatedAt) / 1000 > sensors.sampleInterval)
				) {
					const update = {};
					const sum = {};
					const min = {};
...
deviceSchema.statics.checkReadPerm = async function (deviceID, userId) {
	...
	if (
		userId // public devices do exists -> no user
	)
		return await this.model("Device").exists({
			_id: ObjectId(deviceID),
...
deviceSchema.statics.initControl = async function (createdBy, topic, data, updateTime) {
	...
	const doc = await this.model("Device")
		.findOneAndUpdate(
			...
			{ fields: { "permissions.control": 1, "control.recipe": 1, info: 1 }, new: true }
		)
		.lean();
	if (!doc) throw new Error("invalidPermissions");
...
deviceSchema.statics.getTopicByID = async function (id) {
	const doc = await this.model("Device")
		.findOne(
			{
				_id: ObjectId(id),
