...
export async function processControlData(doc, data) {
	...
	docs.forEach(({ user: userID, control }) => {
		// per USER
...
export async function processSensorsData(doc, data) {
	...
	docs.forEach(({ user: userID, sensors }) => {
		// per USER
...
function processNotifications(userID, data, mode, output, sended, notSended, doc) {
	...
	return ({ JSONkey, type, value: limit, advanced = defaultAdvanced, _id, tmp }) => {
		// per notification rule
		let recipeKey;
		let value;
		if (mode === "control") {
		...
		if (result.ruleSatisfied) {
			if (result.valid) {
				...
				output[userID].push(
					createNotification({ info, recipe: rec, value, homepageUrl, STATEkey: JSONkey }, mode)
				);
...
async function processOutput(output, sended) {
	if (sended.items.length > 0) {
		...
		if (invalidTokens.length) {
			UserModel.removeNotifyTokens(invalidTokens).exec();
