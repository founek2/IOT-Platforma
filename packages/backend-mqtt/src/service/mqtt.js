...
import config from "backend/dist/config"
...
export default (io) => {
    ...
    const client = mqtt.connect('mqtts://localhost', { username: `${config.mqtt.userName}`, password: `${config.mqtt.password}`, port: config.mqtt.port, connectTimeout: 20 * 1000, rejectUnauthorized: false })
    mqttClient = client
