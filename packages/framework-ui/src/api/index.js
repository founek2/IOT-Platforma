...
const processResponse = (dispatch, successMessage) => (response) => {
	...
	if (status === 500) {
		dispatch(
			addNotification({ message: ErrorMessages.getMessage("unexpectedError"), variant: "error", duration: 3000 })
		);
		...
		return response.json().then((json) => {
			...
			if (!dontShow) {
				if (status === 208) {
					const { error } = json;
					dispatch(
						addNotification({ message: ErrorMessages.getMessage(error), variant: "error", duration: 3000 })
					);
					// throw new Error('breakChain');
