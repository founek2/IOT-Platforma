...
import { NOTIFY_INTERVALS } from 'frontend/src/constants'
...
export default function shouldSend({ interval, from, to, daysOfWeek }, tmp = {}) {
    ...
    if (enabled)
        if (interval === NOTIFY_INTERVALS.JUST_ONCE) {
            if (!tmp || !tmp.lastSatisfied)
                return true

        }
        else if (interval === NOTIFY_INTERVALS.ALWAYS || getMinDiff(now, tmp.lastSendAt) > interval)
            return true
