import { signOut } from './api_signIn';
import { getAccessToken, getRefreshToken } from './helpers/accessToken';
...
describe('Refresh API', function () {
    ...
    test('shoud refresh token', async function () {
        const refreshToken = await getRefreshToken(credentials.user)
    ...
    test('shoud fail for access token', async function () {
        const accessToken = await getAccessToken(credentials.user)
